function atlases=ea_genatlastable_2(varargin)
...
if checkrebuild(atlases,options,root,mifix) 
    ...
    for nativemni=nm % switch between native and mni pat.space atlases.
        ...
        for idx=1:length(atlases.names)

          if any(strcmp(names,atlases.names{idx})) || any(strcmp(names,append(atlases.names{idx},'.gz'))) 
            switch atlases.types(idx)
                case 1 % right hemispheric atlas.
                    structure=load_structure([root,filesep,mifix,options.atlasset,filesep,'rh',filesep,atlases.names{idx}],atlases,idx);
                case 2 % left hemispheric atlas.
                    structure=load_structure([root,filesep,mifix,options.atlasset,filesep,'lh',filesep,atlases.names{idx}],atlases,idx);
                case 3 % both-sides atlas composed of 2 files.
                    lstructure=load_structure([root,filesep,mifix,options.atlasset,filesep,'lh',filesep,atlases.names{idx}],atlases,idx);
                    rstructure=load_structure([root,filesep,mifix,options.atlasset,filesep,'rh',filesep,atlases.names{idx}],atlases,idx);
                case 4 % mixed atlas (one file with both sides information).
                    lstructure=load_structure([root,filesep,mifix,options.atlasset,filesep,'mixed',filesep,atlases.names{idx}],atlases,idx,'unmix_l');
                    rstructure=load_structure([root,filesep,mifix,options.atlasset,filesep,'mixed',filesep,atlases.names{idx}],atlases,idx,'unmix_r');
                case 5 % midline atlas (one file with both sides information).
                    structure=load_structure([root,filesep,mifix,options.atlasset,filesep,'midline',filesep,atlases.names{idx}],atlases,idx);
            ...
            for side=detsides(atlases.types(idx))
                if ismember(atlases.types(idx),[3,4,6]) % both-sides atlas composed of 2 files.
                    if side==1
                ...
                if isa(structure,'ea_roi') % volumetric atlas
                    ...
                    try % check if explicit color info for this atlas is available.
                        cdat=atlases.colormap(atlases.colors(idx),:);
                    catch
                    ...
                    ea_addnii2lf(atlases,idx,structure.nii.img,options,root,mifix)
                    
                    iroi{idx,side}=structure; % later stored
                    ifv{idx,side} = [];
                    ipixdim{idx,side}=structure.nii.voxsize(1:3); % later stored
                    iXYZ{idx,side}=XYZ; % later stored
                    try
                        atlases.colors(idx); % check if predefined color exists
                        
                    catch
                        atlases.colors(idx)=idx*(maxcolor/length(atlases.names));
                    end
                ...
                elseif isfield(structure, 'fibers') % fibertract
                    ...
                    iXYZ{idx,side} = structure;

                    ifv{idx,side}=fv;
                    icdat{idx,side}=[];
                    ipixdim{idx,side}='fibers';
                    icolorc{idx,side}=colorc;
                    normals{idx,side}=[];
                    try
                        atlases.colors(idx); % check if predefined color exists
                    catch
                ...
                elseif isfield(structure, 'isdiscfibers') % discriminative fibers
                    ...
                    icdat{idx,side} = [];
                    iXYZ{idx,side} = [];
                    ipixdim{idx,side} = 'discfibers';
                    icolorc{idx,side} = [];
                    normals{idx,side} = [];
                    atlases.colors(idx) = NaN;
                end
                try
                    load([root,filesep,options.atlasset,filesep,'warped_regions.mat'],'atlas');
                   
                    atlas.roi{idx,side} = ea_roi2struct(iroi{idx,side});
                    if  ~any( strcmp(atlas.warped,atlases.names{idx}) )
                        atlas.warped = {atlas.warped{:}, atlases.names{idx}};
                    end
                    destination = append(root,filesep,options.atlasset);
                    matname = fullfile(destination, 'warped_regions.mat');

                    save(matname, 'atlas','-append');
                    %disp('saving roi to warped_regioins.mat')
                %catch ME
                   %disp(ME)
        ...
        %matname = fullfile([root,filesep,mifix,options.atlasset,filesep], ['loaded_areas' '.mat']);
