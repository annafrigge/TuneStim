...
for h = 1:length(hand)
    ...
    if computeDice
        ...
        for j=1:length(clinicalSettings)-1
            diceEnorm{j,h} = dice(dataEnorm{j,h}.d1>=200,dataEnorm{j+1,h}.d1>=200);
            diceEnormTarget{j,h} = dice(dataEnormTarget{j,h}>=200,dataEnormTarget{j+1,h}>=200);
            diceEnormConstraint{j,h} = dice(dataEnormConstraint{j,h}>=200,dataEnormConstraint{j+1,h}>=200);
    ...
    if computeTargetCoverage
        ...
        for j=1:length(clinicalSettings )
            coverageTarget{j,h} = sum(dataEnormTarget{j,h}>=200)/numel(dataEnormTarget{j,h});
            coverageConstraint{j,h} = sum(dataEnormConstraint{j,h}>=200)/numel(dataEnormConstraint{j,h});
