function warp_regions(patientname,root,atlasset,names)
% calls get_target_and_constraint_in native with the options defined in
% getoptslocal

% Input Arguments
% ---------------
% patientname : str, the name of the Patient diretory
% root : str, the path to the patient directory
% atlasset : str, the name of the atlas in which the target and constraint is defined

fprintf('map target and constrain from atlas to native...')
options = getoptslocal;
get_target_constraint_in_native(options,names);

fprintf('done \n')
    function options = getoptslocal

        lead_path = what('lead').path;
        
        options.earoot = lead_path;
        options.root = append(root{1});
        options.patientname = patientname;
        options.atlasset = atlasset;
        options.native = 'True';
        options.atlassetn = 48;
...
% options.earoot = 'C:\Users\annfr888\Documents\MATLAB\lead\';
...
% options.uipatdirs = {'C:\Users\annfr888\Documents\DBS\patient_data\Infinity_Abstract\Infinity_12'};
...
% options.prefs.lc.datadir = 'C:\Users\annfr888\Documents\MATLAB\lead\connectomes\';
...
% options.prefs.machine.normsettings.ants_usefa = 1;
